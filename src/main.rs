...
fn main() {
    ...
    for (__i, __a) in raw_args.iter().enumerate() {
        if __a == "--to-bound-snapshot" {
            if let Some(__v) = raw_args.get(__i + 1) {
                positions.push(__i + 1);
                fields.push(curly_expand::expand_or_literal(__v));
            }
            break;
        } else if let Some(__v) = __a.strip_prefix("--to-bound-snapshot=") {
            positions.push(__i);
            fields.push(
                curly_expand::expand_or_literal(__v)
                    .into_iter()
                    .map(|v| format!("--to-bound-snapshot={}", v))
                    .collect(),
            );
            break;
        }
